...
1. What problem does the context API help solve?
  It allows us to share state between our various components without needing to prop drill. 

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  Actions informs the store of any change in the UI that requires state updates. 
  Reducers, depending on the action, changes the state. 
  Our Store is the central state data that our components point to as a single source of truth for our state. 

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
  Thunk is middleware that allows us to write action-creators that return functions rather than an action.

4. What is your favorite state management system you've learned and this sprint? Please explain why!
  Redux is my favorite as of right now because it seems easier for me to learn. It makes more sense to me to store our state in one place and allows all components to access it rather than passing tons of props down. 